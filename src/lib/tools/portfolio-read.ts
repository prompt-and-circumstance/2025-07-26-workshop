...
export const viewPortfolioTool = tool({
  description: "View current portfolio holdings and investment positions",
  parameters: z.object({
    symbol: z.string().optional().describe("Optional: Filter by specific stock symbol")
  }),
  execute: async ({ symbol }) => {
    ...
    if (symbol) {
      investments = investments.filter(inv => 
        inv.symbol.toLowerCase() === symbol.toLowerCase()
      );
    }
    ...
    const totalValue = investments.reduce((sum, inv) => 
      sum + (inv.shares * (inv.currentPrice || inv.purchasePrice)), 0
    );
    ...
    return {
      ...
      investments: investments.map(inv => ({
        ...
        gainLoss: inv.shares * ((inv.currentPrice || inv.purchasePrice) - inv.purchasePrice),
        gainLossPercent: ((inv.currentPrice || inv.purchasePrice) - inv.purchasePrice) / inv.purchasePrice * 100
      })),
      summary: {
...
export const getClientNotesTool = tool({
  description: "Retrieve client notes and meeting information",
  parameters: z.object({
    clientName: z.string().optional().describe("Optional: Filter notes for specific client")
  }),
  execute: async ({ clientName }) => {
    ...
    return {
      message: `Found ${notes.length} note(s)${clientName ? ` for ${clientName}` : ''}`,
...
export const listTasksTool = tool({
  description: "List pending tasks and action items",
  parameters: z.object({
    showCompleted: z.boolean().optional().default(false).describe("Include completed tasks in results"),
